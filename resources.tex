...
\action{DMLT}{soon}{Should we are all Google Cores for DASK ? Send all PanDA to USDF ?}

Table 40 in \citeds{DMTN-135} allocates 2PB in year 1 growing to 9PB in year 10 for science user home space.
...
\action{DMLT}{soon}{Should we start user space quota at 10GB ?  is there a reason for more ? (or less?)}
...
\action{SQUARE}{soon}{Should we have user home migration to get un touched home spaces moved off cloud?}
